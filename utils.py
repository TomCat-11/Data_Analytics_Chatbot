        ...
        if "histogram" in input_lower or "distribution of" in input_lower:
            ...
            if cols:
                ...
                if pd.api.types.is_numeric_dtype(df[col_y]):
                    ...
                    return f"⚠️ Box plots are best for numerical data. {col_y} is not numeric. Try a bar chart."
            else:
            ...
            if len(cols) == 2:
                ...
                if pd.api.types.is_numeric_dtype(df[col_x]) and pd.api.types.is_numeric_dtype(df[col_y]):
                    ...
                    return f"⚠️ Scatter plots typically require two numerical columns. Please check the data types for {col_x} and {col_y}."
            else:
